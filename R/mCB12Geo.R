...
#' @import graphics
mCB12Geo<-function (x, a, b, k, lambda, method="B")
{
pdf_CB12Geo<-function(par,x){
a=par[1]
b=par[2]
k=par[3]
lambda=par[4]
...
res = suppressWarnings(AdequacyModel::goodness.fit(pdf = pdf_CB12Geo, cdf = cdf_CB12Geo, starts = c(a, b, k, lambda), data = x, method = method, mle = NULL))
