...
#' @import graphics
mCB12NB<-function (x, a, b, k, s, lambda, method="B")
{
pdf_CB12NB<-function(par,x){
a=par[1]
b=par[2]
k=par[3]
s=par[4]
...
res = suppressWarnings(AdequacyModel::goodness.fit(pdf = pdf_CB12NB, cdf = cdf_CB12NB, starts = c(a, b, k, s, lambda), data = x, method = method, mle = NULL))
