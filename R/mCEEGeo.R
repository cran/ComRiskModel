...
#' @import graphics
mCEEGeo<-function (x, alpha, beta, lambda, method="B")
{

pdf_CEEGeo<-function(par,x){
alpha=par[1]
beta=par[2]
lambda=par[3]
...
res = suppressWarnings(AdequacyModel::goodness.fit(pdf = pdf_CEEGeo, cdf = cdf_CEEGeo, starts = c(alpha,beta,lambda), data = x, method = method, mle = NULL))
