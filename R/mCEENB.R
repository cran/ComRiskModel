...
#' @import graphics
mCEENB<-function (x, alpha, beta, s, lambda, method="B")
{

pdf_CEENB<-function(par,x){
alpha=par[1]
beta=par[2]
s=par[3]
lambda=par[4]
...
res = suppressWarnings(AdequacyModel::goodness.fit(pdf = pdf_CEENB, cdf = cdf_CEENB, starts = c(alpha,beta,s,lambda), data = x, method = method, mle = NULL))
