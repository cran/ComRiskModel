...
#' @import graphics
mCLNB<-function (x, b, q, s, lambda, method="B")
{

pdf_CLNB<-function(par,x){
b=par[1]
q=par[2]
s=par[3]
lambda=par[4]
...
res = suppressWarnings(AdequacyModel::goodness.fit(pdf = pdf_CLNB, cdf = cdf_CLNB, starts = c(b, q, s, lambda), data = x, method = method, mle = NULL))
