...
#' @import graphics
mCWBio<-function (x, alpha, beta, m, lambda, method="B")
{
pdf_CWBio<-function(par,x){
alpha=par[1]
beta=par[2]
m=par[3]
lambda=par[4]
...
res = suppressWarnings(AdequacyModel::goodness.fit(pdf = pdf_CWBio, cdf = cdf_CWBio, starts = c(alpha,beta,m,lambda), data = x, method = method, mle = NULL))
