...
#' @import graphics
mCWGeo<-function (x, alpha, beta, lambda, method="B")
{
pdf_CWGeo<-function(par,x){
alpha=par[1]
beta=par[2]
lambda=par[3]
...
res = suppressWarnings(AdequacyModel::goodness.fit(pdf = pdf_CWGeo, cdf = cdf_CWGeo, starts = c(alpha,beta,lambda), data = x, method = method, mle = NULL))
