...
#' @import graphics
mCWNB<-function (x, alpha, beta, s, lambda, method="B")
{
pdf_CWNB<-function(par,x){
alpha=par[1]
beta=par[2]
s=par[3]
lambda=par[4]
...
res = suppressWarnings(AdequacyModel::goodness.fit(pdf = pdf_CWNB, cdf = cdf_CWNB, starts = c(alpha,beta,s,lambda), data = x, method = method, mle = NULL))
